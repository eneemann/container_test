...
#    print(poly_df)
    ...
    pts = gpd.GeoDataFrame(df, crs=crs, geometry = [Point(0, 0)])
    number = n + 10
    min_x, min_y, max_x, max_y = poly_df.total_bounds
    k = 0
    while k < number:
        new = Point(np.random.uniform(min_x, max_x), np.random.uniform(min_y, max_y))
        pt = gpd.GeoDataFrame(df, crs=crs, geometry=[new])

        if neg_buff.all().contains(pt.iloc[0].all()):
        # if pt.within(neg_buff.unary_union):
            pts = pts.append(pt, ignore_index=True)
            k += 1
    
    sample = pts.iloc[1:]       # drop the first row (fake point)
    poly_points = sample
    final = sample
